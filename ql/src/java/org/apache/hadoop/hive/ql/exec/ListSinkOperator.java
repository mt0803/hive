...
public class ListSinkOperator extends Operator<ListSinkDesc> {
  ...
  protected void initializeOp(Configuration hconf) throws HiveException {
	...
	private FetchFormatter initializeFetcher(Configuration conf)
			throws Exception {
		String formatterName = conf.get(OUTPUT_FORMATTER);
		FetchFormatter fetcher;
		if (formatterName != null && !formatterName.isEmpty()) {
			Class<? extends FetchFormatter> fetcherClass = Class.forName(
					formatterName, true,
					Utilities.getSessionSpecifiedClassLoader()).asSubclass(
					FetchFormatter.class);
		...
		String serializationFormat = getConfiguration().get(
				HiveConf.ConfVars.HIVE_LISTSINK_SERIALIZATION_FORMAT.varname);
		if (!StringUtils.isEmpty(serializationFormat)) {
			props.put(serdeConstants.SERIALIZATION_FORMAT,
					"" + Integer.parseInt(serializationFormat));
		} else {
			props.put(serdeConstants.SERIALIZATION_FORMAT, ""
					+ Utilities.tabCode);
		}
		props.put(serdeConstants.SERIALIZATION_NULL_FORMAT, getConf()
